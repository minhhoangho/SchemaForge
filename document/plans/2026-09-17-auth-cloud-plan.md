...
| 30 | **Spec mục 2 "Thông báo lỗi và dò tài khoản" yêu cầu lỗi `429` (`too-many-requests`) hiện "Thử lại sau N phút" (đếm ngược dùng `ApiFailure.retryAfterSeconds`), nhưng Task 27 chỉ yêu cầu hiện đúng thông báo chung đã dịch theo mã lỗi, không đếm ngược.** | Task 27 (dòng "credentials-form.tsx") liệt kê `too-many-requests` cùng nhóm với `invalid-credentials`, `email-already-registered`, `password-too-common`: "hiện đúng thông báo của mã", không nhắc `retryAfterSeconds` hay đếm ngược. `apiErrors` hiện không có key với placeholder số phút. | (a) (đề xuất, người dùng đã chọn 2026-09-25) Giữ nguyên Task 27 như đã cài đặt: chỉ hiện thông báo chung, không đếm ngược. Đây là điểm thu hẹp so với spec, chấp nhận để không tăng phạm vi Task 27. (b) Thêm đếm ngược theo spec: cần sửa `apiErrors` thêm key có `{{minutes}}` và `credentials-form.tsx` đọc `failure.retryAfterSeconds` — không chọn. | Task 27 | Đã chốt 2026-09-25: phương án (a), người dùng xác nhận qua orchestrator. |
| 31 | **Mock `usePathname` chưa được thêm vào file helper test chung** | Task 30 thêm stub `usePathname: () => "/schemas",` vào mock `next/navigation` trong bốn file hành trình editor test (`frontend/src/journey/*.test.tsx`); file helper chung `frontend/src/testing/mount-editor-journey.test.tsx` đã được xác định cần sửa y hệt nhưng bị chặn bởi hệ thống quyền orchestrator; ba test trong file này hiện thất bại: "holds the schema lock in the shared registry while an editor is mounted", "opens a guest schema under a signed-out auth provider without a request", "mounts the editor in the requested locale", đều lỗi `No "usePathname" export is defined on the "next/navigation" mock` | Xác định trong quá trình triển khai Task 30: `AccountMenu` gọi `usePathname()` là đúng và theo chuẩn; hệ thống quyền từ chối fix 2 dòng trong file helper lần thứ hai nhưng lần thứ ba không thử lại | (a) **(chấp nhận)** Khi được phép, thêm đúng 2 dòng vào mock `vi.mock("next/navigation", …)` ở đầu file: thêm comment `// usePathname from next/navigation` (nếu cần) và `usePathname: () => "/schemas",` sau dòng hiện tại `useRouter` ở đó, khớp với cách sửa đã làm ở bốn file journey. Không cần thay đổi plan. (b) Mở rộng plan hiện tại bằng task `mount-editor-journey-mock-fix`: không cần, lỗi là về cơ sở hạ tầng test, không logic sản xuất | Task 30 (thử chưa được) | Chấp nhận (quyết định của orchestrator, không qua người dùng): lỗi hạ tầng test, không ảnh hưởng phạm vi phần 4 |
