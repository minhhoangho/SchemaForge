...
- `editor-workspace.tsx` (sửa từ Task 22) trở thành bố cục đầy đủ. Hiện workspace chưa có landmark nào (toolbar và canvas nằm trong một `<div>` xếp dọc); task này thêm `<header>`, hai `<aside>`, `<main>` và skip link (Vấn đề 63):
  ...
  - `useFocusRequest(...)` và `<CreateRelationDialog draft onClose />`; `onConnect` của canvas (Task 22 để hàm rỗng) gọi `createRelationDraftFromConnection`, nút "Thêm quan hệ" của panel bảng gọi `createRelationDraftFromTable`.
