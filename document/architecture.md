...
| Kiểm tra hình dạng trong core | Zod, runtime dependency của entry chính `packages/core`; `@dbml/core` chỉ được import trong subpath importer (`@schemaforge/core/importers/sql`, `.../dbml`), tải lazy trong Web Worker | AI SDK khai báo tool bằng Zod, nên hình dạng operation chỉ có một nguồn; entry chính và bundle editor không kéo theo `@dbml/core` |
