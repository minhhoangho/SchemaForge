...
| 8 | Spec mục 2 gọi ô chọn kiểu cột là "combobox có ba nhóm". Combobox của shadcn/ui là `Popover` + `Command`, cần thêm gói `cmdk` không có trong bảng phiên bản | **(cần user xác nhận)** Dùng `Select` của Radix với `SelectGroup` và `SelectLabel` cho ba nhóm (17 kiểu chung, enum, "Kiểu custom…"), không thêm dependency; truy cập bằng bàn phím có sẵn. Nếu cần gõ để lọc thì phải thêm `cmdk` qua một thay đổi spec và một task dependency riêng | 3, 26 |
