...
- **Môi trường máy dev (ghi sau đợt 12, Vấn đề 69).** Một hook của shell chặn `source ~/.nvm/nvm.sh` và viết lại lệnh `git`. Khi gặp, dùng tiền tố `PATH=$(ls -d ~/.nvm/versions/node/v24*/bin | tail -1):$PATH` thay cho `source ~/.nvm/nvm.sh …; nvm use 24 …`, và gọi `/usr/bin/git` thay cho `git`. Chạy `test`, `build` và `format:check` của frontend trong **một** lệnh gộp từng bị hệ điều hành kill vì hết bộ nhớ (exit 137), nên chạy từng lệnh riêng.
...
| `eslint.config.mjs` | Chỉ Task 2 sửa ở lượt này. Task B cần đổi rule thì dừng và báo; orchestrator tạo một task cấu hình riêng, chạy tuần tự. Không tắt rule bằng comment để né việc này, trừ các chỗ plan ghi rõ (kèm lý do): `@next/next/no-sync-scripts` trong `layout.tsx` (Vấn đề 41), `@typescript-eslint/no-implied-eval` trong `theme-init-script.test.ts` (Task 10), `@typescript-eslint/no-deprecated` cho `keyCode` trong `committed-text-field.tsx` (Vấn đề 56) |
...
- `shouldHandleShortcut` trả `false` khi:
  ...
  - `isDialogOpen`;
  - target nằm trong phần tử `[role="dialog"]` hoặc `[role="alertdialog"]` (thêm ở Task 29, Vấn đề 81);
...
| 46 | **(đã chấp nhận sau đợt 10)** Task 20 ghi `requestSave` giữ `isSaving`, `pendingDocument` trong `useRef` và chỉ nói "cleanup hủy đăng ký", không nói gì về lần ghi đang chạy khi unmount. Nhưng unmount nhả khóa tab, nên tab khác hoặc màn hình danh sách có thể ghi schema này ngay sau đó | Hàng đợi (`createAutosaver`) được tạo **bên trong effect**; cleanup hủy đăng ký `store.subscribe` rồi `dispose()`. Sau unmount không ghi thêm gì: `pendingDocument` bị bỏ, lần ghi đang chạy xong thì không đặt `saveStatus`, không hiện toast (kể cả khi lỗi), và `retry` (cả nút "Thử lại" trong toast) không làm gì. Việc lưu là **một vòng `while`** trong `save`: ghi xong thì lấy `pendingDocument` mới nhất và ghi tiếp, `isSaving` về `false` cùng tick với lần lấy cuối, nên không thay đổi nào rơi giữa hai lần ghi. `notify` đọc qua `useRef` để đổi ngôn ngữ không tạo hàng đợi thứ hai | 20, 22, 29 |
| 47 | **(đã chấp nhận sau đợt 10)** Task 20 ghi "`grantId` tăng mỗi lần được cấp". Thực tế `grantId` chỉ là `1` (khóa trống, lấy ngay) hoặc `2` (được cấp sau khi chờ), không phải bộ đếm | Không cần bộ đếm: giữa hai trạng thái `held` luôn có `acquiring` hoặc `blocked` (state gắn với `schemaId` và `lockManager` của lần yêu cầu, đổi một trong hai thì đọc ra `acquiring`), nên Task 22 truyền `grantId` (là `null` khi chưa `held`) cùng `schemaId` làm dependency của `useOpenSchema` vẫn đọc lại tài liệu sau mỗi lần được cấp. **Lưu ý cho Task 22:** ở dev, React StrictMode chạy effect hai lần; yêu cầu `ifAvailable` thứ hai gặp khóa của yêu cầu thứ nhất chưa kịp nhả, nên mở schema thoáng hiện `blocked` ("đang mở ở tab khác") rồi `held` với `grantId: 2`, và tài liệu bị đọc hai lần. Bản production không bị. Task 22 quyết định có làm mượt hay không và ghi lại cách xử lý trong báo cáo; **không** được giải quyết bằng timer trong logic (ví dụ trì hoãn màn hình `blocked`) | 20, 22, 32 |
| 48 | **(đã chấp nhận sau đợt 10)** Task 21 ghi `useSchemaList` trả `readonly SchemaListEntry[] \| undefined`. Nhưng `useLiveQuery` ném lại lỗi của query ngay trong render, Dexie chỉ báo lỗi mở database ở query đầu tiên, và route `/` không có error boundary | `useSchemaList(repository): SchemaListResult \| undefined` với `SchemaListResult = { kind: "loaded"; entries } \| { kind: "failed"; errorCode: StorageErrorCode }`; `undefined` vẫn là đang đọc. Query tự bắt lỗi, ghi `logger.error("schema-list.read-failed", { errorName })` và trả `failed` để màn hình hiện thông báo `storage:<errorCode>` đã dịch. `getSchemaListEntryId(entry)` được export cùng file | 21 |
| 49 | **(đã chấp nhận sau đợt 10)** Task 21 ghi `deleteSchema: (schemaId) => Promise<void>`, nên màn hình không biết dòng (và nút menu mà focus sẽ quay về) còn tồn tại hay không | `SchemaActions.deleteSchema` trả `Promise<boolean>`, `true` chỉ khi đã xóa (khóa bị giữ hoặc repository lỗi đều trả `false`). Hộp thoại xóa `await` kết quả và chỉ đóng sau đó: xóa được thì focus về `<h1>` của màn hình (`tabIndex={-1}`), bị từ chối thì focus về nút menu hành động của dòng. `createSchema`, `renameSchema` vẫn trả `Promise<void>` | 21 |
| 50 | **(đã chấp nhận sau đợt 10)** Task 21 thiếu file và key so với code thực tế: màn hình vượt giới hạn kích thước hàm của `.claude/rules/code-quality.md` nếu giữ state hộp thoại và focus trong `schema-list-screen.tsx` | Thêm vào "File sở hữu" của Task 21: `features/schema-list/hooks/use-schema-list-dialogs.ts` (hộp thoại nào đang mở, focus trả về đâu khi đóng: nút đã mở, hoặc `<h1>` nếu phần tử đó không còn) và `features/schema-list/components/schema-list-dialogs.tsx` (ba hộp thoại tạo, đổi tên, xóa). Thêm key `schemaList`: `create.description`, `rename.description`, `delete.unreadableTitle` (tiêu đề hộp thoại xóa cho dòng không đọc được, không có tên) | 21 |
| 51 | **(đã chấp nhận sau đợt 10)** jsdom không có IndexedDB toàn cục, và `liveQuery` của Dexie bỏ qua mọi query khi không tìm thấy IndexedDB, nên test dùng `useLiveQuery` không bao giờ nhận dữ liệu dù database được tạo với `fake-indexeddb` | Test của `useSchemaList` và `SchemaListScreen` đặt `Dexie.dependencies.indexedDB = new IDBFactory()` và `Dexie.dependencies.IDBKeyRange = IDBKeyRange` (từ `fake-indexeddb`) trong `beforeAll`. Vitest mặc định `isolate: true` (`frontend/vitest.config.ts` không ghi đè), nên thay đổi này chỉ sống trong file test đó. Task sau có test dùng `useLiveQuery` (ví dụ Task 31) làm giống vậy, không thêm vào `setup-tests.ts` | 21, 31 |
| 52 | **(đã chấp nhận sau đợt 10)** Spec mục 7 "Đổi tên và xóa từ màn hình danh sách" ghi "Xóa: lấy khóa, sau khi người dùng xác nhận thì xóa bản ghi". Code lấy khóa **sau** khi người dùng xác nhận | Giữ như code: `deleteSchema` chạy `tryAcquire` khi bấm nút xác nhận, rồi xóa và nhả khóa trong `finally`. Lý do: giữ khóa trong lúc hộp thoại đang mở sẽ chặn editor ở tab khác mở schema đó trong suốt thời gian người dùng suy nghĩ. Kết quả với người dùng không đổi (khóa bị giữ thì toast "đang mở ở tab khác" và không xóa). Không sửa spec; Task 32 ghi lệch này khi đối chiếu tiêu chí | 21, 32 |
| 53 | **(đã chấp nhận sau đợt 10)** Task 23 ghi nút tên schema mang `aria-invalid="true"` khi tên có issue. ARIA không cho phép `aria-invalid` trên role `button` | Nút tên schema không dùng `aria-invalid`. Tên có issue thì nút hiện icon cảnh báo (`aria-hidden`), tooltip chứa các thông báo đã dịch, và `aria-describedby` trỏ tới một danh sách ẩn chứa cùng thông báo. Issue của tên là các `schemaIssues` có `path` đúng bằng `["name"]`. Nút là `DialogTrigger` nên Radix trả focus về nút khi hộp thoại đóng; `Tooltip` luôn được render (chỉ `TooltipContent` phụ thuộc issue) để phần tử nút không bị mount lại khi issue xuất hiện hay biến mất. State mở hộp thoại nằm trong `SchemaNameButton` | 23, 29 |
| 54 | **(đã chấp nhận sau đợt 10)** Task 23 ghi nút đếm issue mang `aria-label={t("issues.count", { count })}`, làm tên truy cập không chứa đúng chữ hiển thị (WCAG 2.5.3) | Khi có issue: số hiển thị nằm trong `<span aria-hidden>` và cụm `t("toolbar.issues.count", { count })` nằm trong `<span className="sr-only">`, không có `aria-label`. Khi 0 issue: nút chỉ có icon, `size="icon"`, `aria-label={t("toolbar.issues.none")}`. Key `issues.count` có `count_one`, `count_other` (file `vi` giữ cả hai để bộ key khớp `en`) | 23 |
| 55 | **(đã chấp nhận sau đợt 10)** Task 23 không nói trạng thái lưu có được đọc lên hay không; nếu chữ hiển thị là live region thì mỗi lần autosave đều bị đọc | Chữ hiển thị "Đang lưu", "Đã lưu", "Lưu thất bại" **không** phải live region. Một `<span role="status" className="sr-only">` luôn được mount và chỉ có nội dung khi `failed`, nên trình đọc màn hình chỉ thông báo lỗi lưu | 23 |
| 56 | **(đã chấp nhận sau đợt 10)** Task 23 chỉ bỏ qua Enter khi `nativeEvent.isComposing`. Safari báo `isComposing === false` ở phím Enter kết thúc một cụm gõ Telex, VNI, nên Enter đó commit ô nhập giữa chừng | `CommittedTextField` bỏ qua Enter khi `nativeEvent.isComposing` **hoặc** `nativeEvent.keyCode === 229`. `keyCode` đã deprecated, nên dòng đó có `eslint-disable-next-line @typescript-eslint/no-deprecated -- keyCode is the only signal Safari gives for the Enter that ends an IME composition`. Đây là ngoại lệ tắt rule bằng comment được plan cho phép, cùng loại với Vấn đề 41 (mục "Điểm nóng khi làm song song", dòng `eslint.config.mjs`); không file nào khác được tắt `@typescript-eslint/no-deprecated`. Task 32 kiểm tra tay trên Safari | 23, 26, 32 |
| 57 | **(đã chấp nhận sau đợt 10)** Spec mục 10 đòi chuyển động khung nhìn bằng 0 khi `prefers-reduced-motion: reduce`, nhưng plan không nói chỗ nào xử lý | `ViewportControls` **không** xử lý reduced motion; nó dùng đúng `duration` được truyền. Mọi nơi gọi `setCenter` tự tính `duration` là `window.matchMedia("(prefers-reduced-motion: reduce)").matches ? 0 : VIEWPORT_TRANSITION_MS`: `useSchemaCommands` (Task 23, đã làm), `useRevealFocusedElement` (Task 24), `useRevealTable` (Task 25), và bất kỳ chỗ gọi mới nào ở Task 29 | 23, 24, 25, 29 |
| 58 | **(đã chấp nhận sau đợt 10)** Task 23 mô tả `addTable` là "`{ x: 0, y: 0 }` cộng offset của `findFreeTablePosition`" rồi mới dựng operation, không rõ vị trí để căn giữa lấy ở đâu | `addTable` gọi `findFreeTablePosition(document, { x: 0, y: 0 })` **trước** khi dispatch, truyền đúng vị trí đó cho `buildAddTableOperation`, và `setCenter` tới chính vị trí đó (giữ `getZoom()`). Dispatch không thành công thì không chọn, không focus, không di chuyển khung nhìn | 23 |
| 59 | **(đã chấp nhận sau đợt 11)** Task 24 ghi `EditorCanvas` tự bọc `ReactFlowProvider` và tự dựng `ViewportControlsProvider` bên trong. Nhưng toolbar (zoom, fit view) và lệnh thêm bảng (`useSchemaCommands` gọi `setCenter`) nằm **cạnh** canvas, không nằm trong nó, nên `useViewportControls` của chúng throw khi Task 22 ghép toolbar với canvas | Hai provider được tách ra `frontend/src/features/editor/components/canvas/editor-flow-provider.tsx`: `EditorFlowProvider({ children })` bọc `ReactFlowProvider`, rồi một component con dựng `ViewportControls` từ `useReactFlow()` (thời gian chuyển động là `0` khi `prefers-reduced-motion: reduce`) và bọc `ViewportControlsProvider`. `EditorCanvas` không còn tự bọc provider nào và phải được render bên trong `EditorFlowProvider`. `EditorWorkspace` render `EditorStoreProvider` → `EditorFlowProvider` → toolbar và canvas. Test `editor-canvas.test.tsx`, `table-node.test.tsx`, `relation-edge.test.tsx` của Task 24 bọc canvas trong `EditorFlowProvider`. **Task 29 phải biết:** panel trái (`useRevealTable` gọi `useViewportControls`), panel thuộc tính, hộp thoại tạo quan hệ và mọi phần tử mới gọi `useViewportControls` đều phải nằm **bên trong** `EditorFlowProvider`; bố cục landmark được dựng bên trong provider | 22, 24, 25, 29 |
| 60 | **(đã chấp nhận sau đợt 11)** Task 22 đặt `EditorSkeleton` là component không export trong `editor-screen-loader.tsx`, `loading.tsx` "không có chữ", và không nói trạng thái "đang mở" được đọc lên thế nào. Skeleton bị mount lại ở mỗi bước mở (storage, khóa, đọc tài liệu), nên một live region nằm trong nó sẽ được đọc chập chờn; màn hình trạng thái thay skeleton ở client thì focus rơi về `<body>` | `EditorSkeleton` nằm trong file riêng `features/editor/components/editor-skeleton.tsx`, chỉ để nhìn (`aria-hidden`, không có chữ). **Một** live region `<p role="status" className="sr-only">` nằm trong `EditorScreenLoader`, **ngoài** ranh giới `next/dynamic`, chứa `t("screen.loading")` khi đang mở và rỗng khi xong. `EditorScreen` nhận thêm prop `onOpeningChange: (isOpening: boolean) => void`: `true` khi storage `pending`, khi đang lấy khóa hoặc đang đọc; `false` khi `blocked`, khi đã mở hoặc khi hiện một màn hình trạng thái. `loading.tsx` (Server Component) render `<p role="status" className="sr-only">` với `t("screen.loading")` từ `getServerTranslation` cùng `<EditorSkeleton />`. `EditorStatusScreen` và `error.tsx` focus `<h1>` (`tabIndex={-1}`) khi mount để trình đọc màn hình đọc ngay trạng thái mới | 22, 32 |
| 61 | **(đã chấp nhận sau đợt 11)** Những điểm khác của Task 22 so với plan: `error.tsx` chỉ có nút "Tải lại"; `generateMetadata` nhận `params`; `storage-unavailable` luôn lấy mô tả từ `storage:<code>`, trong khi `storage:unknown` ("Không lưu được. Hãy thử lại.") nói về lần **lưu**, sai với schema không **đọc** được | `error.tsx` có thêm `<Link href="/">` `t("screen.backToList")` cạnh nút `common:actions.reload`. `generateMetadata()` không nhận tham số (server không đọc IndexedDB nên tiêu đề không phụ thuộc `schemaId`) và truyền `{ appName: APP_NAME }` cho `t("screen.pageTitle")`. Thêm key `editor.screen.storageUnavailable.readFailed`; `EditorStatusScreen` dùng key này khi `storageErrorCode` là `unknown` (cũng là giá trị mặc định của prop), mã khác vẫn dùng `storage:<code>`. `useOpenSchema` bắt mọi lỗi ném ra khi đọc tài liệu hoặc viewport (ví dụ `DatabaseClosedError`) thành `storage-error`, nên màn hình "Không dùng được bộ nhớ trình duyệt" cũng hiện khi đọc thất bại (spec mục 1 đã thêm dòng này). State của `useOpenSchema` ghi kèm `repository`, `schemaId`, `grantId` của lần đọc, nên khi một trong ba đổi thì hook trả `opening` cho tới khi lần đọc mới xong. `EditorWorkspace` chưa nối `onConnect` (hàm rỗng tới Task 29) và không ghi viewport sau unmount | 22, 29 |
| 62 | **(đã chấp nhận sau đợt 11)** Test `ignores a result that arrives after unmount` của Task 22 không kiểm chứng được: sau unmount thật thì không còn gì để quan sát việc bỏ kết quả | Thay bằng `ignores a result that arrives after its read was cleaned up`: đổi `grantId` về `null` (effect bị dọn), trả kết quả muộn, rồi cấp lại cùng `grantId`; hook vẫn phải là `opening`. Task 22 cũng tạo thêm `app/schemas/[schemaId]/error.test.tsx` (không có trong "File sở hữu") và `editor-workspace.test.tsx` (6 test `it` và một `it.each`, Vấn đề 63), cùng các test ngoài danh sách ghi ở mục "Test viết trước" của Task 22 | 22 |
| 63 | **(đã chấp nhận sau đợt 11)** Task 29 ghi **tạo** `editor-workspace.test.tsx`, nhưng Task 22 đã tạo file này cho phần toolbar và canvas. Workspace hiện chưa có landmark nào: toolbar và canvas nằm trong một `<div>` xếp dọc | Task 29 **sửa** `editor-workspace.test.tsx`: giữ các test đang có, gồm 6 test `it` và một `it.each` (`zooms react flow from the toolbar`, `adds a table from the empty state and saves it`, `uses the stored viewport as the default viewport`, `saves the viewport when a move ends`, `logs a failed viewport save without showing it`, `saves no viewport after unmount`, và `reports no axe violations in the %s theme`), thêm test của Task 29. Task 29 thêm `<header>`, hai `<aside>`, `<main>` và skip link | 22, 29 |
| 64 | **(đã chấp nhận sau đợt 11)** Task 29 tìm trường cần focus theo `data-focus-path`, nhưng Task 25, 26, 27 không nói mỗi trường mang đường dẫn nào; đường dẫn tự đặt sẽ không khớp `issue.path` mà tab "Vấn đề" gửi vào `requestFocus` | **Quy tắc chung cho mọi panel** (spec mục 4 "Tab Vấn đề"): mỗi trường nhập liệu có thể có issue mang `data-focus-path` bằng `JSON.stringify` của **đúng** đường dẫn issue của core cho trường đó. `CommittedTextField`, `CommittedTextArea` và `LabeledCheckbox` nhận prop `focusPath?: DocumentPath`; phần tử khác đặt thẳng `data-focus-path`. Panel bảng: `["tables", id, "name" \| "comment"]`; `["columns", id, "name" \| "isNullable" \| "isUnique" \| "isAutoIncrement" \| "comment"]`; nút mở combobox kiểu `["columns", id, "type"]`; tham số kiểu `["columns", id, "type", "length" \| "precision" \| "scale" \| "name"]`; giá trị mặc định `["columns", id, "defaultValue"]` đặt trên ô giá trị khi mặc định là literal, ngược lại trên radio đang chọn; `["indexes", id, "name" \| "isUnique"]`. Checkbox khóa chính không mang đường dẫn. Phần "Chi tiết" của cột **tự mở** khi cột có issue dưới `type`, `defaultValue` hoặc `comment` (kể cả khi issue xuất hiện lúc đang mở panel), và nhãn nút đổi thành `t("tablePanel.columns.detailsHasIssues")` ("Chi tiết (có vấn đề)"). **Giới hạn đã chấp nhận:** người dùng tự đóng "Chi tiết" trong khi issue vẫn còn thì yêu cầu focus tới trường bên trong không tìm thấy phần tử và không làm gì (đúng hành vi "không tìm thấy thì không làm gì" của `useFocusRequest`) | 25, 26, 27, 29 |
| 65 | **(đã chấp nhận sau đợt 11)** Task 26 thiếu file, key và quy tắc so với code thực tế: các file trong "File sở hữu" vượt giới hạn kích thước của `.claude/rules/code-quality.md` nếu giữ hết logic | Thêm vào "File sở hữu" của Task 26, trong `panels/table-panel/`: `column-default-field.tsx`, `custom-type-form.tsx`, `icon-action-button.tsx`, `index-column-list.tsx`, `index-item.tsx`, `labeled-checkbox.tsx`, `list-move-focus.ts`, `use-display-name.ts`, `use-field-error-message.ts`. Thêm key `editor.tablePanel`: `position.label`, `unnamed`, `columns.customTypeEntry`, `columns.applyCustomType`, `columns.detailsHasIssues`, `columns.currentType`, `indexes.columnsLabel`, `indexes.chooseColumn`. Quy tắc: ô vị trí rỗng hoặc không phải số hữu hạn, và ô độ dài, precision, scale rỗng hoặc không phải số nguyên đạt giá trị tối thiểu của core thì **không dispatch** (ô quay lại giá trị đã lưu). Giá trị mặc định đã lưu mà kiểu mới không còn cho phép vẫn nằm trong danh sách radio (đang được chọn), để radio khớp tài liệu và issue giải thích lý do. Ô tìm kiếm kiểu dùng `filter` riêng của cmdk (`scoreColumnTypeSearch` trong `features/editor/lib/column-type-options.ts`): so chuỗi con không phân biệt hoa thường trên `keywords` là chữ hiển thị, không so `value`, nên id của enum không khớp tìm kiếm. Mục đang dùng có chữ `sr-only` `t("tablePanel.columns.currentType")` ("(đang dùng)"). Thuật ngữ `vi`: "Ghi chú" cho comment, "Kiểu thông dụng" cho nhóm 17 kiểu chung, "Kiểu tự đặt…" cho mục kiểu custom | 26, 32 |
| 66 | **(đã chấp nhận sau đợt 11)** Mở một schema có thể gọi `onMoveEnd` của React Flow (và ghi viewport) mà người dùng chưa pan hay zoom | Chấp nhận, không chặn: lần ghi chỉ đụng bảng `viewports`, không đổi tài liệu, không đổi `updatedAt` và không đưa schema lên đầu danh sách. Spec mục 7 "Viewport" đã ghi hành vi này | 22, 24 |
| 67 | **(đã chấp nhận sau đợt 12)** Task 30 ghi test `renders only the changed column row` đếm `ColumnRow` bên trong `TableNode` bằng `Profiler`. `Profiler` không bọc được các dòng mà `TableNode` tự render nếu không sửa code production | Test đó render trực tiếp các `ColumnRow` của bảng bị sửa, mỗi dòng bọc trong một `Profiler` riêng, rồi khẳng định chỉ dòng của cột bị sửa render lại. Việc `TableNode` render thừa được bắt ở test thứ nhất `renders no other table node when one column is renamed`, so số lần render giữa hai bảng | 30 |
| 68 | **(đã chấp nhận sau đợt 12)** Task 30 không nói script `perf:snippet` nạp fixture TypeScript thế nào. Import tĩnh có đuôi `.ts` làm `typecheck` fail với TS5097 (không bật `allowImportingTsExtensions`); import không có đuôi thì Node không tìm thấy module | `scripts/print-large-schema-snippet.ts` dùng `import type` cho kiểu của `src/testing/large-schema.ts`, rồi `import()` động một URL tính bằng `new URL("../src/testing/large-schema.ts", import.meta.url)`, kiểm lại bằng type guard `isLargeSchemaModule`. Lệnh `node --experimental-strip-types` chạy được trên Node 24 nên script giữ đúng như plan. Đoạn JavaScript in ra **dừng** khi database `schemaforge` chưa tồn tại (hủy `onupgradeneeded` và in hướng dẫn), nên người đo phải mở màn hình danh sách schema một lần trước khi dán. Lệnh kiểm tra `perf:snippet \| head -5` của Task 30 in lỗi `EPIPE` vì `head` đóng pipe sớm; script không có lỗi | 30, 32 |
| 69 | **(ghi sau đợt 12)** Trên máy dev, một hook của shell chặn `source ~/.nvm/nvm.sh` và viết lại lệnh `git`; chạy `test`, `build`, `format:check` của frontend trong một lệnh gộp bị kill vì hết bộ nhớ (exit 137) | Agent dùng `PATH=$(ls -d ~/.nvm/versions/node/v24*/bin \| tail -1):$PATH` và `/usr/bin/git`, và chạy từng lệnh kiểm tra riêng. Đã ghi vào mục "Quy ước chung cho mọi task" cho các task sau | 29, 31, 32 |
| 70 | **(user đã chốt: sửa core cho khớp spec)** Spec mục 3 "Tạo quan hệ" ghi chế độ "Tạo cột mới" dựng một cột khóa ngoại cho **mỗi cột được tham chiếu** (thả vào cột `d` thì tham chiếu `d`). Nhưng `buildRelation` của core luôn dùng khóa chính của bảng đích, nên kéo thả vào một cột không phải khóa chính sẽ tạo sai khóa ngoại | Core (commit c4efa92): `RelationInput` có thêm `referencedColumnIds?: readonly ColumnId[]`. Bỏ trống thì dùng khóa chính như cũ; có truyền thì dựng một cột cho mỗi id theo đúng thứ tự, 1-1 một cột thì `isUnique`, nhiều cột thì index unique. Lỗi theo thứ tự: `table-not-found`; bỏ trống và bảng đích không có khóa chính thì `primary-key-missing`; mảng rỗng thì `invalid-shape` tại `["referencedColumnIds"]`; `column-not-found`, `column-not-in-table`, `column-listed-twice` tại `["referencedColumnIds", i]`. Có truyền id thì bảng đích không cần khóa chính; cột được tham chiếu không unique cho issue `relation-target-not-unique`. Cột khóa ngoại nullable khi `onDelete` hoặc `onUpdate` là `setNull` (hành vi của core, spec mục 3 ghi "không nullable" cho hộp thoại); hộp thoại luôn dùng `noAction`. Task 28 truyền `referencedColumnIds` của draft cho `buildRelation`. Spec phần 2 mục 9 và plan phần 2 Task 29 đã cập nhật | 28, core |
| 71 | **(đã chấp nhận sau đợt 12)** Task 28 chỉ sở hữu một file component, nên `create-relation-dialog.tsx` vượt giới hạn kích thước của `.claude/rules/code-quality.md`; danh sách key cũng thiếu so với code thực tế | Owner đã duyệt tách thành các file trong `components/dialogs/`: `create-relation-dialog.tsx`, `relation-draft-form.tsx`, `relation-draft-fields.tsx`, `relation-tables-field.tsx`, `foreign-key-fields.tsx`, `referenced-columns-field.tsx`, `column-pairs-field.tsx`, `junction-name-field.tsx`, `dialog-select-field.tsx`, `dialog-radio-field.tsx`, `field-error-message.tsx`, `use-relation-field-errors.ts` (kèm `use-relation-field-errors.test.ts`), `relation-dialog-fields.ts`. Key `editor.relationDialog` thêm: `description` tách thành `description` và `enterHint` (`enterHint` chỉ hiện khi xác nhận được), `referencedColumnLabel`, `columnPairs.title`, `columnPairs.placeholder`, `columnPairs.toLabel` (dòng gợi ý "references {{column}}", `vi` "tham chiếu {{column}}"), `errors.primaryKeyMissingManyToMany` | 28 |
| 72 | **(đã chấp nhận sau đợt 12)** Task 28 ghi `onClose` trả focus về node nguồn và Task 29 truyền hàm này xuống. Nhưng `FocusScope` của Radix hoàn tác focus được đặt trong `onClose` | Hộp thoại tự trả focus trong `onCloseAutoFocus` của `DialogContent`. Khi mở (`onOpenAutoFocus`), hộp thoại ghi lại `document.activeElement` và `fromTableId`. Khi đóng: phần tử đó là control thật (khác `body`, còn trong DOM, ví dụ nút "Thêm quan hệ") thì để Radix trả focus về nó; ngược lại `preventDefault` rồi focus `.react-flow__node` có `dataset.id` bằng id bảng nguồn (so bằng `dataset`, không dựng selector từ id); node không còn thì focus `.react-flow` → `closest("[tabindex]")`. Task 29 **chỉ** đặt draft về `null` trong `onClose` | 28, 29, 32 |
| 73 | **(yêu cầu mới cho Task 29, sau đợt 12)** Đường dự phòng của Vấn đề 72 tìm phần tử có `tabindex` bao quanh `.react-flow`. Workspace hiện chưa có phần tử nào như vậy, nên focus rơi về `body` | Task 29 **phải** render vùng canvas là `<main tabIndex={-1}>` bọc `.react-flow` (tức bọc `<EditorCanvas />`). Cách này cũng khớp spec mục 12: xóa phần tử thì focus về vùng canvas. **Đã làm ở Task 29 (đợt 13, commit 02f01ae):** `editor-workspace.tsx` render `<main tabIndex={-1} aria-label={t("layout.canvasLabel")}>` bọc skip link và `<EditorCanvas />`; test `renders the canvas region as a focusable main around react flow` phủ yêu cầu này | 29 |
| 74 | **(đã chấp nhận sau đợt 12)** Task 28 không nói focus ban đầu của hộp thoại nằm ở đâu, trong khi spec mục 3 muốn "kéo xong bấm Enter là đủ" | Focus ban đầu nằm trên nút xác nhận khi nút đang bật, nên Enter chấp nhận giá trị mặc định. Nút bị `disabled` thì focus control đầu tiên có `aria-invalid="true"` (mô tả của nó nói lý do bị chặn), không có thì focus chính nội dung hộp thoại | 28 |
| 75 | **(đã chấp nhận sau đợt 12)** Task 28 không nói lỗi được đọc lên thế nào, và lỗi `Result` từ core hiện "dưới trường" nào | Lỗi của `validateRelationDraft` được đọc qua **một** vùng `<p role="status" className="sr-only">` luôn có mặt. Lỗi core trả khi xác nhận hiện với `role="alert"`; trước mỗi lần xác nhận lại, lỗi cũ được xóa bằng `flushSync` để cùng một lỗi được đọc lại. Lỗi được xếp theo `OperationError.path`: bắt đầu bằng `referencedColumnIds` thì hiện dưới "Cột được tham chiếu", còn lại hiện dưới phần chọn bảng (`errorFieldOfOperationError`). Core không bao giờ từ chối tên bảng trung gian (tên sai thành issue của schema), nên không có nhánh lỗi cho ô tên bảng trung gian | 28, 32 |
| 76 | **(đã chấp nhận sau đợt 12)** Những điểm khác của Task 28 so với plan | `validateRelationDraft` báo lỗi `{ field: "referencedColumnIds", reason: "duplicate-column" }` ở **cả hai** chế độ khóa ngoại (core từ chối cột được tham chiếu lặp), nên `RelationDraftError` được mở rộng: nhánh `referencedColumnIds` có `reason: "primary-key-missing" \| "duplicate-column"`. Dòng cột được tham chiếu và cặp cột có `key` theo chỉ số. `onDelete`, `onUpdate` không có trên hộp thoại và luôn là `noAction`. Danh sách bảng "to" gồm cả bảng `from`, nên tạo được quan hệ tự tham chiếu. `generateId` dùng `crypto.randomUUID()` ngay trong component, như `use-schema-commands.ts` | 28 |
| 77 | **(chưa lên lịch, phát hiện ở đợt 12)** `frontend/src/features/schema-list/components/create-schema-dialog.tsx` và `rename-schema-dialog.tsx` hiện thông báo lỗi mà không có live region, nên trình đọc màn hình không đọc lỗi (WCAG 4.1.3 Status Messages) | Sửa trong một task riêng, chưa xếp đợt: dùng cùng cách của Vấn đề 75 (một vùng `role="status"` luôn có mặt cho lỗi kiểm tra, `role="alert"` cho lỗi khi xác nhận) | Task riêng |
| 78 | **(đã chấp nhận sau đợt 13)** Task 29 đặt `useEditorShortcuts`, `useDeleteSelection` và `useFocusRequest` thẳng trong `editor-workspace.tsx`, nên component bố cục vượt giới hạn kích thước và lẫn logic với JSX | Tách file `frontend/src/features/editor/hooks/use-workspace-keyboard.ts` (không có trong plan): `useWorkspaceKeyboard(isDialogOpen: boolean): { setCanvasRegion, deleteSelection }`. `setCanvasRegion` là callback ref của `<main>`, giữ phần tử trong state; hook nối `useDeleteSelection(focusCanvas)`, `useEditorShortcuts({ store, canvasElement, isDialogOpen, platformHint: navigator.platform, onDeleteSelection })` và `useFocusRequest(focusFieldByPath)`. Đọc `navigator.platform` lúc render là an toàn vì `EditorScreen` được nạp với `ssr: false`. `WorkspaceLayout` gần như chỉ còn JSX; state của hộp thoại tạo quan hệ nằm trong hàm `useRelationDialog` không export của `editor-workspace.tsx`. Hook không có file test riêng, được phủ qua `editor-workspace.test.tsx` | 29 |
| 79 | **(đã chấp nhận sau đợt 13)** Task 29 ghi `useDeleteSelection(): () => void` và toast gọi thẳng `undo()` sau khi lưu `previousHistoryLength`. Hook không có cách tự đưa focus về vùng canvas, còn `undo()` trần sẽ hoàn tác một thay đổi khác nếu người dùng sửa tiếp trước khi bấm "Hoàn tác" | `useDeleteSelection(focusCanvas: () => void): () => void` nhận hàm focus làm tham số (`use-workspace-keyboard.ts` truyền hàm focus `<main>`). Ngay sau `dispatch`, hook giữ **tham chiếu** `history.past.at(-1)`; nút "Hoàn tác" của toast chỉ gọi `undo()` khi `history.past.at(-1)` vẫn là đúng mục đó (so identity), ngược lại không làm gì. Không dùng `previousHistoryLength`: độ dài lịch sử không đổi khi một mục mới đẩy mục cũ nhất ra khỏi giới hạn lịch sử. Test thêm: `leaves a later change alone when the toast action runs` | 29 |
| 80 | **(đã chấp nhận sau đợt 13)** Spec mục 3 muốn toast "Hoàn tác" và focus về vùng canvas sau **cả hai** cách xóa, nhưng Task 26, 27 cho panel tự dispatch, tự xóa lựa chọn rồi gọi `onDeleted`; panel quan hệ còn không có prop nào. Như vậy xóa bằng nút không có toast, và có ba bản sao của đường xóa | `TablePanel`, `RelationPanel`, `MultiSelectionPanel` và `PropertiesPanel` nhận `onDelete: () => void` thay cho `onDeleted`, không còn tự dispatch hay xóa lựa chọn. Workspace truyền `deleteSelection` của Vấn đề 78 xuống, vì mỗi nút xóa ở đây xóa đúng lựa chọn hiện tại. Mọi lần xóa (phím `Delete` hay nút trong panel) đi qua một đường: một `dispatch`, xóa lựa chọn, focus `<main>`, toast "Hoàn tác". Chữ `<RelationPanel relationId />` và `onDeleted` ở Task 26, 27, 29 đã lỗi thời. Test đổi: `table-panel.test.tsx` `asks to delete the table` (thay `removes the table`); `relation-panel.test.tsx` `asks to delete the relation` (thay `removes the relation and clears the selection`); `multi-selection-panel.test.tsx` `asks to delete the whole selection` thay `deletes everything with one dispatch`, `restores everything with a single undo`, `clears the selection after deleting`, `keeps the selection when the deletion is rejected`; các test này chuyển sang `use-delete-selection.test.tsx` với độ phủ tương đương | 26, 27, 29 |
| 81 | **(đã chấp nhận sau đợt 13)** Task 29 ghi `isDialogOpen` phủ cả hộp thoại đổi tên schema. Nhưng state mở của hộp thoại đó nằm trong `SchemaNameButton` (Task 23), workspace không đọc được | `shouldHandleShortcut` (`lib/should-handle-shortcut.ts`) trả `false` thêm khi target nằm trong phần tử `[role="dialog"]` hoặc `[role="alertdialog"]` (dùng `closest`). Cách này phủ hộp thoại đổi tên và mọi hộp thoại, popover Radix sau này mang role đó. `isDialogOpen` vẫn giữ và chỉ phản ánh hộp thoại tạo quan hệ. Test thêm: `should-handle-shortcut.test.ts` `ignores shortcuts from a button inside a %s element` (`it.each` `dialog`, `alertdialog`); `editor-workspace.test.tsx` `does not undo while focus is on a button of the rename dialog` dùng hộp thoại đổi tên thật | 8, 29 |
| 82 | **(đã chấp nhận sau đợt 13)** Task 28 ghi `createRelationDraftFromTable` trả `null` khi tài liệu không có đủ hai bảng, nên nút "Thêm quan hệ" (đường thay thế bằng bàn phím và bấm cho thao tác kéo) không mở được gì trong schema một bảng, dù kéo nối vẫn tạo được quan hệ tự tham chiếu | Sửa trong `lib/to-relation-draft.ts` (file của Task 28): không có bảng khác thì `toTable` là chính bảng `from` (quan hệ tự tham chiếu). Vẫn trả `null` với id bảng không có trong tài liệu. Test đổi: `returns null when the document has no other table` thành `builds a self relation when the document has no other table`; thêm `returns null for a table the document does not hold` | 28, 29 |
| 83 | **(đã chấp nhận sau đợt 13)** Task 29 không nói phần tử nào nhận focus từ skip link, và vòng focus hiện thế nào trên vùng nhận focus bằng `tabIndex={-1}` | Skip link trỏ tới `<aside>` của panel thuộc tính (`id`, `tabIndex={-1}`) khi có lựa chọn; không có lựa chọn thì trỏ tới một `<div tabIndex={-1}>` bọc `<LeftPanel />`, vì `left-panel.tsx` không thuộc Task 29. Vòng focus của `<main>` và div bọc là lớp phủ `::after` (`relative`, `focus-visible:after:absolute inset-0 z-10 ring-2 ring-inset ring-ring pointer-events-none`), không phải box-shadow inset, vì con có nền (`.react-flow`, `<aside>`) che box-shadow. Skip link dùng `focus:px-3 focus:py-2`, vì `focus:not-sr-only` đặt lại padding. Test thêm: `moves focus to the left panel through the skip link without a selection`. Việc chuyển `id`, `tabIndex` sang `<aside>` của `LeftPanel` ghi ở Vấn đề 87 | 29, 32 |
| 84 | **(đã chấp nhận sau đợt 13)** Task 29 ghi key `deleted.many` là một chuỗi, nhưng câu `en` cần số ít và số nhiều | Key `editor.layout`: `skipToPanel`, `propertiesLabel`, `canvasLabel`, `deleted.one`, và `deleted.many` dùng key số nhiều của i18next `many_one`, `many_other`. Tiếng Việt chỉ có một dạng số nhiều nên i18next chỉ đọc `many_other`; key `many_one` của `vi` chỉ để hai bộ key bằng nhau (`satisfies LocaleNamespace`). Code vẫn truyền hai key hằng `editor:layout.deleted.one` và `editor:layout.deleted.many` kèm `count`. Test thêm: `use-delete-selection.test.tsx` `counts the elements in the Vietnamese toast` | 29 |
| 85 | **(đã chấp nhận sau đợt 13)** Task 29 không nói test kiểm tra toast thế nào, và chưa có test ở mức workspace cho nút xóa của panel quan hệ và panel nhiều lựa chọn | Test của hook và của workspace dùng `Toaster` thật của Sonner và gọi `toast.dismiss()` trong `afterEach`, vì Sonner giữ toast trong state cấp module. Test workspace thêm `deletes the table from the table panel with an undo toast`; chưa có test bấm nút xóa của panel quan hệ hay panel nhiều lựa chọn ở mức workspace. Không bắt buộc; gợi ý Task 31 phủ đầu cuối (toast và "Hoàn tác") | 29, 31 |
| 86 | **(chưa lên lịch, phát hiện ở đợt 13)** Toast của Sonner mặc định tắt sau 4000 ms và không dừng khi focus bằng bàn phím, nên khó Tab tới nút "Hoàn tác" kịp. WCAG 2.2.1 Timing Adjustable vẫn đạt vì hoàn tác được bằng `Ctrl/Cmd+Z`, nút Undo trên toolbar và Alt+T | Sửa trong một task riêng, chưa xếp đợt: ví dụ thêm tùy chọn `duration` dài hơn cho toast có action trong `frontend/src/lib/notify.ts` | Task riêng |
| 87 | **(chưa lên lịch, phát hiện ở đợt 13)** Đích của skip link khi chưa chọn gì là `<div tabIndex={-1}>` bọc `LeftPanel` (Vấn đề 83), không phải chính landmark `<aside>` của panel trái | Sửa trong một task riêng, chưa xếp đợt: chuyển `id`, `tabIndex={-1}` và lớp vòng focus sang `<aside>` trong `frontend/src/features/editor/components/panels/left-panel.tsx`, rồi bỏ div bọc | Task riêng |
| 88 | **(đã chấp nhận sau đợt 14)** Chữ ký `mountEditor` của Task 31 không có `onOpeningChange`, trong khi `EditorScreen` bắt buộc prop này; Task 31 cũng ghi hành trình 8 tự dựng hai `SchemaLockManager` | Helper truyền một hàm không làm gì cho `onOpeningChange`. Mỗi lần gọi `mountEditor` tự dựng một `SchemaLockManager` riêng từ `lockRegistry` dùng chung, giống một tab trình duyệt; hành trình 8 chỉ gọi `mountEditor` hai lần. `mountSchemaList()` luôn dùng `en`; `mountEditor` mặc định `en` khi không truyền `locale` | 22, 31 |
| 89 | **(đã chấp nhận sau đợt 14)** jsdom không bao giờ đổi kích thước node, nên React Flow không đo lại và edge của cột khóa ngoại mới thêm (`users_id`) không được vẽ | Helper thay `ResizeObserver` bằng một bản báo kích thước khi `observe` và khi nội dung đổi (theo dõi bằng `MutationObserver`), và spy getter `offsetHeight` (20 px cho mỗi dòng `li`). Spec mục 14 cho phép mock `ResizeObserver`. Mỗi file test hoàn tác bằng `vi.unstubAllGlobals()` và `vi.restoreAllMocks()` trong `afterEach`. Việc đo lại thật trên trình duyệt vẫn nằm trong checklist kiểm tra tay của Task 32 | 31, 32 |
| 90 | **(đã chấp nhận sau đợt 14)** jsdom không pan hay zoom được, nên hành trình 4 không kích hoạt được `onMoveEnd` bằng thao tác thật | File của hành trình 4 bọc `@xyflow/react` để ghi lại props (`onMoveEnd`, `defaultViewport`, `fitView`, `colorMode`) mà vẫn render `ReactFlow` thật, cùng cách của `editor-workspace.test.tsx` và `editor-canvas.test.tsx`. Pan, zoom thật dẫn tới `onMoveEnd` vẫn nằm trong checklist kiểm tra tay của Task 32 | 31, 32 |
| 91 | **(đã chấp nhận sau đợt 14)** Task 31 không nói tài liệu ban đầu của các hành trình không bắt đầu từ schema rỗng được dựng thế nào | Hành trình quan hệ và hành trình bàn phím dựng tài liệu bằng các factory của `@schemaforge/core/testing` rồi lưu qua `repository.saveDocument` thật | 31 |
| 92 | **(đã chấp nhận sau đợt 14)** Hành trình 7 chỉ dùng bàn phím; một vòng `user.tab()` không giới hạn sẽ treo test khi control cần tới không có | Vòng Tab nằm trong helper (`tabTo`, `shiftTabOutOfTextFields`) có giới hạn 80 điểm dừng, nên thiếu control thì test fail thay vì treo. Trong jsdom `navigator.platform` không phải Mac nên Mod = Ctrl, và phím chọn nhiều của React Flow là Control. Hành trình 7 không gọi `user.click`; vì schema dựng bằng bàn phím chỉ có một bảng, bước tạo quan hệ tạo quan hệ tự tham chiếu `users.users_id → users.id` (Vấn đề 82). Web Locks thật giữa hai tab vẫn nằm trong checklist kiểm tra tay của Task 32. Helper truy vấn dùng chung (như `readDocument`, truy vấn dòng outline và tên edge) nằm trong `frontend/src/testing/` | 31, 32 |
| 93 | **(đã chấp nhận sau đợt 14)** Task 31 không nói `Dexie.dependencies` được trỏ tới `fake-indexeddb` ở đâu | Đặt trong `beforeAll` của hai file có mount danh sách schema (`schema-list-journey.test.tsx`, `keyboard-and-locking.test.tsx`), theo cách sửa của Vấn đề 51, không đặt trong helper | 31 |
| 94 | **(đã chấp nhận sau đợt 14)** Ngoài danh sách test của Task 31, cần phủ gợi ý của Vấn đề 85 | `relations.test.tsx` thêm hai test đầu cuối: `deletes the relation from the relation panel with an undo toast` và `deletes a table and a relation from the multi-selection panel with an undo toast`. Vấn đề 85 nay đã được phủ | 29, 31 |
| 95 | **(chưa lên lịch, phát hiện ở đợt 14)** Hai test hành trình `ignores Delete inside a text field` và `ignores Delete while a dialog is open` không tách riêng được nhánh chặn ô nhập liệu và hộp thoại của `shouldHandleShortcut`: `use-editor-shortcuts.ts` đã đòi focus nằm trong canvas mới xử lý `Delete`, còn ô nhập liệu của panel và hộp thoại không bao giờ nằm trong canvas. Hai nhánh này được phủ bằng unit test trong `should-handle-shortcut.test.ts` | Gợi ý Task 32 ghi rõ điểm này. Một hành trình chỉ dùng bàn phím với hai bảng cũng sẽ kiểm tra được nhiều hơn quan hệ tự tham chiếu của Vấn đề 92 | 31, 32 |
...
- `viewport-controls.tsx` (`"use client"`):
  ...
  - `useViewportControls` ngoài provider throw `Error`. `EditorFlowProvider` (`components/canvas/editor-flow-provider.tsx`, Task 22, Vấn đề 59) dựng `controls` từ `useReactFlow`; toolbar và panel trái chỉ dùng interface này, nên test mock được mà không cần `ReactFlowProvider`.
...
- `use-autosave.ts`:
  ...
  - Một effect tạo hàng đợi ghi rồi `store.subscribe` so `state.document` với giá trị trước; đổi tham chiếu thì gọi `requestSave(document)`. Cleanup hủy đăng ký và `dispose()` hàng đợi: sau unmount không ghi thêm gì, không đặt `saveStatus`, không toast, `retry` không làm gì (Vấn đề 46).
  - Hàng đợi giữ `isSaving` và `pendingDocument`. Đang ghi thì chỉ ghi `pendingDocument` rồi thoát. Không thì đặt `saveStatus: { kind: "saving" }` và chạy một vòng `while`: `await repository.saveDocument(schemaId, document)`, rồi lấy `pendingDocument` mới nhất và ghi tiếp cho tới khi hết; hết thì `saveStatus: { kind: "saved" }`.
...
- `use-schema-lock.ts`:
  ...
  - Không: state `blocked`, rồi `acquire(schemaId, controller.signal)`; khi được cấp thì `{ kind: "held", grantId: 2 }`. `grantId` chỉ là `1` hoặc `2`, không phải bộ đếm; giữa hai lần `held` luôn có `acquiring` hoặc `blocked`, nên Task 22 dùng `grantId` làm dependency vẫn đọc lại tài liệu (Vấn đề 47, kể cả lưu ý về StrictMode ở dev).
...
- Tạo trong `frontend/src/features/schema-list/components/`: `schema-list-screen.tsx`, `schema-list-screen.test.tsx`, `schema-list-row.tsx`, `create-schema-dialog.tsx`, `create-schema-dialog.test.tsx`, `rename-schema-dialog.tsx`, `delete-schema-dialog.tsx`, `schema-list-dialogs.tsx` (Vấn đề 50).
- Tạo trong `frontend/src/features/schema-list/hooks/`: `use-schema-list.ts`, `use-schema-list.test.tsx`, `use-schema-actions.ts`, `use-schema-actions.test.tsx`, `use-schema-list-dialogs.ts` (Vấn đề 50).
...
- `use-schema-list.ts`: `useSchemaList(repository: SchemaRepository): SchemaListResult | undefined` dùng `useLiveQuery` của `dexie-react-hooks` gọi `repository.listSchemas()`, với `SchemaListResult = { kind: "loaded"; entries } | { kind: "failed"; errorCode }`. `undefined` nghĩa là đang đọc; lỗi đọc được bắt trong query, không ném trong render (Vấn đề 48).
- `use-schema-actions.ts`:
  ...
  export type SchemaActions = {
    ...
    // true only when the schema was deleted (Vấn đề 49)
  ...
  - `renameSchema` và `deleteSchema` gọi `lockManager.tryAcquire(schemaId)` trước (với xóa là sau khi người dùng đã xác nhận, Vấn đề 52); `null` thì `notify({ tone: "error", titleKey: "schemaList:openInAnotherTab" })` và không làm gì; ngược lại làm việc rồi `lock.release()` trong `finally`.
...
- `delete-schema-dialog.tsx`: `AlertDialog` với cảnh báo không hoàn tác được, nút hủy và nút xóa. Nút xóa `await` `deleteSchema`; xóa được thì focus về `<h1>` (`tabIndex={-1}`), bị từ chối thì về nút menu của dòng (Vấn đề 49).
- Key `schemaList` cần có: `pageTitle`, `title`, `create.trigger`, `create.dialogTitle`, `create.nameLabel`, `create.submit`, `rename.*`, `delete.title`, `delete.unreadableTitle`, `delete.description`, `delete.confirm`, `create.description`, `rename.description`, `empty.title`, `empty.createFirst`, `row.actions`, `row.open`, `row.rename`, `row.delete`, `row.unreadable`, `row.updatedAt`, `nameRequired`, `openInAnotherTab`, `unreadableCannotRename`, `loading`.

**Test viết trước** (dùng `renderWithProviders`, bọc thêm `<StorageProvider storage={fakeStorage}>` với repository thật trên `fake-indexeddb` và `createFakeLockRegistry`; `vi.mock("next/navigation")`; test có `useLiveQuery` đặt `Dexie.dependencies` trong `beforeAll`, Vấn đề 51):
...
- Tạo `frontend/src/app/schemas/[schemaId]/page.tsx`, `loading.tsx`, `error.tsx`, `page.test.tsx`, `error.test.tsx` (Vấn đề 62).
- Tạo trong `frontend/src/features/editor/components/`: `editor-screen-loader.tsx`, `editor-screen.tsx`, `editor-screen.test.tsx`, `editor-skeleton.tsx` (Vấn đề 60), `editor-status-screen.tsx`, `editor-status-screen.test.tsx`, `editor-workspace.tsx`, `editor-workspace.test.tsx` (Vấn đề 63).
- Tạo `frontend/src/features/editor/components/canvas/editor-flow-provider.tsx`; sửa `canvas/editor-canvas.tsx`, `editor-canvas.test.tsx`, `table-node.test.tsx`, `relation-edge.test.tsx` của Task 24 để bỏ provider khỏi canvas (Vấn đề 59).
...
- `app/schemas/[schemaId]/page.tsx` (Server Component, default export):
  ...
  `await params`; `isSchemaId(schemaId)` sai thì render `<EditorStatusScreen variant="not-found" />`, đúng thì render `<EditorScreenLoader schemaId={schemaId} />`. `generateMetadata` không nhận tham số và trả `t("screen.pageTitle", { appName: APP_NAME })` của namespace `editor` (không chứa tên schema, vì server không đọc IndexedDB; Vấn đề 61).
- `loading.tsx` (default export, Server Component): `<p role="status" className="sr-only">` chứa `t("screen.loading")` (dịch bằng `getServerTranslation`) cùng `<EditorSkeleton />` (Vấn đề 60).
- `editor-skeleton.tsx`: `EditorSkeleton()` là khung `Skeleton` của toolbar và canvas, chỉ để nhìn: `aria-hidden`, không có chữ, `motion-reduce:animate-none`.
- `error.tsx` (`"use client"`, default export): props `{ readonly error: Error; readonly reset: () => void }`; hiện `t("editor:screen.crashTitle")` trong `<h1 tabIndex={-1}>` được focus khi mount, `crashDescription`, nút `common:actions.reload` gọi `reset()` và `<Link href="/">` `t("screen.backToList")` (Vấn đề 60, 61). Không hiển thị `error.message`; `logger.error("editor.crashed", { errorName: error.name })`.
- `editor-screen-loader.tsx` (`"use client"`): `const EditorScreen = dynamic(() => import("./editor-screen").then((module) => module.EditorScreen), { ssr: false, loading: () => <EditorSkeleton /> })`. Loader phải là client component vì Next.js 16 không cho `ssr: false` trong Server Component. `EditorSkeleton` import từ `./editor-skeleton`. Loader giữ `const [isOpening, setIsOpening] = useState(true)`, render **một** `<p role="status" className="sr-only">` (có `t("screen.loading")` khi `isOpening`, rỗng khi không) nằm ngoài ranh giới `dynamic`, rồi `<EditorScreen schemaId onOpeningChange={setIsOpening} />` (Vấn đề 60).
...
- `editor-status-screen.tsx` (`"use client"`): một component cho mọi trạng thái không mở được.
  ...
  Render `<main>` có `<h1 tabIndex={-1}>` được focus khi mount (Vấn đề 60), mô tả và `<Link href="/">` về danh sách. Key lấy theo `variant` từ một object hằng ánh xạ `variant` sang cặp key (không ghép chuỗi key động, để `ParseKeys` còn kiểm tra được). `storage-unavailable` lấy mô tả từ `storage:<storageErrorCode>`, trừ khi mã là `unknown` (giá trị mặc định của prop) thì dùng `screen.storageUnavailable.readFailed` (Vấn đề 61).
- `use-open-schema.ts`:
  ...
  - Effect chạy lại mỗi khi `grantId` đổi (tab này vừa được cấp khóa sau khi chờ), gọi `Promise.all([repository.openSchema(schemaId), repository.readViewport(schemaId)])`, bỏ kết quả nếu effect đã bị hủy (cờ `isActive` trong cleanup). State ghi kèm `repository`, `schemaId`, `grantId` của lần đọc; khác input hiện tại thì hook trả `opening` (Vấn đề 61).
...
- `editor-screen.tsx` (`"use client"`), props `{ readonly schemaId: string; readonly onOpeningChange: (isOpening: boolean) => void }` (Vấn đề 60):
...
- `editor-workspace.tsx` (`"use client"`), props `{ readonly schemaId: string; readonly document: SchemaDocument; readonly viewport: ViewportRecord | null; readonly repository: SchemaRepository }`:
  ...
  - Render `<EditorStoreProvider store={store}>` → `<EditorFlowProvider>` (Vấn đề 59) bọc một `<div>` xếp dọc: `<EditorToolbar onRetrySave={autosave.retry} />` rồi `<EditorCanvas defaultViewport={...} onMoveEnd={...} onAddTable={...} onConnect={...} />`; `onConnect` là hàm rỗng tới Task 29.
  - `onMoveEnd` gọi `repository.saveViewport({ schemaId, x, y, zoom })` và nuốt lỗi bằng `logger.warn("editor.viewport-save-failed", { errorName })`: mất viewport không đáng làm hỏng phiên làm việc. Sau unmount không ghi viewport (khóa tab đã nhả). Lần ghi do React Flow gọi `onMoveEnd` khi mở schema là chấp nhận được (Vấn đề 66).
...
- Key `editor.screen` cần có: `pageTitle`, `notFound.title`, `notFound.description`, `locked.title`, `locked.description`, `unsupportedVersion.title`, `unsupportedVersion.description`, `unreadable.title`, `unreadable.description`, `storageUnavailable.title`, `storageUnavailable.readFailed` (Vấn đề 61), `crashTitle`, `crashDescription`, `backToList`, `loading`.

**Test viết trước:**

- `page.test.tsx`: `renders the not found state for an id that is not a uuid`; `renders the editor loader for a valid id`; `translates the page title in %s`.
- `error.test.tsx`: `shows a translated crash message without the error message`; `resets the route when reload is pressed`; `links back to the schema list`; `moves focus to the heading when it mounts`; `logs only the name of the error`; `reports no axe violations in the %s theme`.
- `editor-status-screen.test.tsx`: `shows the %s message` (`it.each` năm variant); `links back to the schema list`; `shows the translated storage message for an unavailable storage`; `describes an unknown storage error as a failed read`; `moves focus to the heading when it mounts`; `reports no axe violations in the %s theme`.
- `editor-screen.test.tsx` (repository thật trên `fake-indexeddb`, `createFakeLockRegistry`):
  ...
  - `keeps one loading status mounted until the schema is open`.
  - `stops the loading status and focuses the heading of a state that replaces the skeleton`.
  ...
  - `reports no axe violations in the %s theme` (`it.each` `light`, `dark`).
- `use-open-schema.test.tsx`: `stays in opening while no lock has been granted`; `reads the document and the viewport together`; `re-reads the document when a new grant arrives`; `ignores a result that arrives after its read was cleaned up` (thay cho `ignores a result that arrives after unmount`, Vấn đề 62); `ignores a result for a grant that was replaced`; `reports not found for a schema that was never stored`; `marks an unreadable document with %s`; `maps a thrown dexie error to a storage error`.
- `editor-workspace.test.tsx` (Vấn đề 63): `zooms react flow from the toolbar`; `adds a table from the empty state and saves it`; `uses the stored viewport as the default viewport`; `saves the viewport when a move ends`; `logs a failed viewport save without showing it`; `saves no viewport after unmount`; `reports no axe violations in the %s theme`.
...
- `committed-text-field.tsx` (`"use client"`):
  ...
  - Commit khi `blur` và khi `keydown` `Enter`, trừ khi `event.nativeEvent.isComposing` hoặc `event.nativeEvent.keyCode === 229` (Safari, Vấn đề 56; dòng `keyCode` có comment tắt `@typescript-eslint/no-deprecated` được phép). `Escape` đặt `draft` về `value` và **không** commit.
...
- `use-schema-commands.ts`:
  ...
  - `addTable`: `ViewportControls` không cho biết tâm khung nhìn hiện tại, nên `position = findFreeTablePosition(document, { x: 0, y: 0 })` được tính **trước** khi dispatch (Vấn đề 58). Gọi `buildAddTableOperation(document, { position, generateId: () => crypto.randomUUID() })`, `dispatch(result.operation)` (không thành công thì dừng), `setSelection({ tableIds: [result.tableId], relationIds: [] })`, `requestFocus(["tables", result.tableId, "name"])`, và `setCenter(position.x, position.y, { zoom: getZoom(), duration })` với `duration` là `0` khi `matchMedia("(prefers-reduced-motion: reduce)").matches`, ngược lại `VIEWPORT_TRANSITION_MS` (`ViewportControls` không tự xử lý reduced motion, Vấn đề 57).
...
- `editor-toolbar.tsx` (`"use client"`), props `{ readonly onRetrySave: () => void }`, thứ tự đúng spec:
  ...
  2. `<SchemaNameButton />`: nút hiện `document.name`, mở `Dialog` đổi tên dùng `CommittedTextField`; xác nhận dispatch `{ type: "renameSchema", name }`. Nút là `DialogTrigger` để focus quay về nó khi hộp thoại đóng. Tên schema có issue (`schemaIssues` có `path` đúng bằng `["name"]`) thì nút hiện icon cảnh báo, thông báo đã dịch nằm trong tooltip và trong danh sách ẩn mà `aria-describedby` trỏ tới; **không** dùng `aria-invalid` (Vấn đề 53).
  ...
  6. `<IssueCountButton />`: `getIssueIndex(document).issues.length`; bằng 0 thì nút chỉ có icon đạt, không có số, `aria-label={t("issues.none")}`; khác 0 thì icon cảnh báo kèm số hiển thị (`aria-hidden`) và cụm `t("issues.count", { count })` trong `sr-only`, không `aria-label` (Vấn đề 54); bấm thì `setLeftPanelTab("issues")`.
  7. `<SaveStatusBadge />`: `common:saveStatus.saving`, `saved`, `failed`; `failed` kèm nút `common:actions.retry` gọi `onRetrySave`. Chữ hiển thị không phải live region; một `role="status"` `sr-only` luôn mount chỉ thông báo khi `failed` (Vấn đề 55).
...
- `editor-canvas.tsx` (`"use client"`), props `{ readonly defaultViewport: Viewport | null; readonly onMoveEnd: (viewport: Viewport) => void; readonly onAddTable: () => void; readonly onConnect: (connection: Connection) => void }`:
  - **Đã đổi ở Task 22 (Vấn đề 59):** `EditorCanvas` không tự bọc provider. `ReactFlowProvider` và `ViewportControlsProvider` (dựng `ViewportControls` từ `useReactFlow()`) nằm trong `EditorFlowProvider` (`components/canvas/editor-flow-provider.tsx`); `EditorWorkspace` bọc toolbar, canvas và (từ Task 29) hai panel trong provider này, và `EditorCanvas` phải được render bên trong nó.
...
**Test viết trước** (component test bọc `ReactFlowProvider`, `EditorStoreProvider` và `ViewportControlsProvider` giả; từ Task 22, `editor-canvas.test.tsx`, `table-node.test.tsx` và `relation-edge.test.tsx` bọc canvas trong `EditorStoreProvider` → `EditorFlowProvider`, Vấn đề 59):
...
- Tạo trong `frontend/src/features/editor/components/panels/table-panel/`: `table-panel.tsx`, `table-panel.test.tsx`, `table-position-fields.tsx`, `column-list.tsx`, `column-list.test.tsx`, `column-item.tsx`, `column-type-combobox.tsx`, `column-type-combobox.test.tsx`, `column-details.tsx`, `column-details.test.tsx`, `index-list.tsx`, `index-list.test.tsx`, và các file tách ra khi làm (Vấn đề 65): `column-default-field.tsx`, `custom-type-form.tsx`, `icon-action-button.tsx`, `index-column-list.tsx`, `index-item.tsx`, `labeled-checkbox.tsx`, `list-move-focus.ts`, `use-display-name.ts`, `use-field-error-message.ts`.
...
- `column-type-options.ts`:
  ...
  Đúng 17 kiểu chung. `buildColumnType` giữ lại tham số cũ khi đổi giữa hai kiểu cùng tham số (`char` ↔ `varchar` giữ `length`), ngược lại dùng giá trị mặc định. File này còn export `scoreColumnTypeSearch(value, search, keywords)`, `filter` của cmdk cho ô tìm kiếm kiểu (Vấn đề 65).
...
- `table-panel.tsx` (`"use client"`), props `{ readonly tableId: TableId }`:
  ...
  - `<TablePositionFields />`: hai `CommittedTextField` `inputMode="numeric"` hiện `Math.round(position.x)`, `Math.round(position.y)`, trong `<fieldset>` có `<legend>` `t("tablePanel.position.label")`. Commit: ô rỗng (sau khi bỏ khoảng trắng) hoặc `Number(value)` không phải số hữu hạn thì **không dispatch** (ô tự quay lại giá trị cũ; `Number("")` là `0` nên ô rỗng phải được loại riêng); hợp lệ thì dispatch một `moveElements` chỉ chứa bảng này.
  - Nút "Thêm quan hệ" gọi prop `onCreateRelation(tableId)`; nút "Xóa bảng" dispatch `removeTable` rồi `setSelection(EMPTY_SELECTION)` và gọi prop `onDeleted()`. Hai prop do Task 29 truyền vào. **Đã đổi ở Task 29 (Vấn đề 80):** prop là `onDelete: () => void`; nút "Xóa bảng" chỉ gọi `onDelete()`, không tự dispatch hay xóa lựa chọn. Test `removes the table` đổi thành `asks to delete the table`.
  - Trường có issue nhận `errorMessage` dịch từ `issuesOfElement(tableId)` khớp phần `path` sau id phần tử (`useFieldErrorMessage`).
  - Mọi trường có thể có issue mang `focusPath` hoặc `data-focus-path` bằng đúng đường dẫn issue của core (quy tắc chung cho mọi panel, Vấn đề 64).
- `column-list.tsx` và `column-item.tsx`: mỗi cột một dòng theo `table.columnIds` (`key` là `columnId`):
  ...
  - Nút "Chi tiết" (`aria-expanded`, `aria-controls`) mở `<ColumnDetails />`. Phần này tự mở khi cột có issue dưới `type`, `defaultValue` hoặc `comment` (`hasIssueUnder`), kể cả khi issue xuất hiện sau khi render, và nhãn nút khi đó là `t("tablePanel.columns.detailsHasIssues")`. Người dùng vẫn tự đóng được; đóng khi issue còn thì yêu cầu focus tới trường bên trong không tìm thấy gì (Vấn đề 64).
...
- `column-type-combobox.tsx`: `Popover` + `Command` của shadcn/ui (`CommandInput`, `CommandList`, `CommandEmpty`, `CommandGroup`, `CommandItem`), ba nhóm: `t("columns.typeGroups.common")` (17 kiểu, `vi` "Kiểu thông dụng"), `t("columns.typeGroups.enums")` (mọi enum trong `sortEnums`), `t("columns.typeGroups.custom")` (một mục `t("columns.customTypeEntry")`, `vi` "Kiểu tự đặt…", mở `CustomTypeForm` với nút `t("columns.applyCustomType")` ngay trong popover). Nút kích hoạt có tên truy cập `t("columns.typeLabel", { column })` (chữ `sr-only`), hiện `formatColumnType` và mang `data-focus-path` `["columns", id, "type"]`.
  - `Command` nhận `filter={scoreColumnTypeSearch}`: so chuỗi con không phân biệt hoa thường trên `keywords` (chữ hiển thị), không so `value`; `value` của mục enum là id nên id không được khớp tìm kiếm. Mục đang dùng có chữ `sr-only` `t("columns.currentType")` ("(đang dùng)") bên cạnh dấu tick chỉ để nhìn (Vấn đề 65).
...
- `column-details.tsx`: tham số kiểu (`length`, hoặc `precision` và `scale`, hoặc tên kiểu custom) bằng `CommittedTextField`; ô số rỗng hoặc không phải số nguyên đạt giá trị tối thiểu của core thì không dispatch. `RadioGroup` giá trị mặc định (`column-default-field.tsx`) với các lựa chọn `t("columns.default.none")` cộng `getAllowedColumnDefaults(type)`, cộng loại mặc định đã lưu nếu kiểu hiện tại không còn cho phép (vẫn được chọn, issue giải thích); chọn `literal` thì hiện thêm ô giá trị; `CommittedTextArea` comment của cột (Vấn đề 65).
...
- Thuật ngữ `vi`: "Ghi chú" cho comment (`commentLabel` "Ghi chú bảng", `columns.comment` "Ghi chú cột"), "Kiểu thông dụng", "Kiểu tự đặt" (Vấn đề 65).
- Key `editor.tablePanel` cần có (rút gọn): `label`, `nameLabel`, `commentLabel`, `unnamed`, `position.label`, `position.x`, `position.y`, `position.hint`, `addRelation`, `removeTable`, `columns.title`, `columns.add`, `columns.nameLabel`, `columns.typeLabel`, `columns.typeGroups.common`, `columns.typeGroups.enums`, `columns.typeGroups.custom`, `columns.customTypeEntry`, `columns.customTypeLabel`, `columns.applyCustomType`, `columns.search`, `columns.noResult`, `columns.nullable`, `columns.primaryKey`, `columns.unique`, `columns.autoIncrement`, `columns.moveUp`, `columns.moveDown`, `columns.remove`, `columns.details`, `columns.detailsHasIssues`, `columns.currentType`, `columns.default.label`, `columns.default.none`, `columns.default.literal`, `columns.default.currentTimestamp`, `columns.default.generateUuid`, `columns.default.valueLabel`, `columns.length`, `columns.precision`, `columns.scale`, `columns.comment`, `indexes.title`, `indexes.add`, `indexes.nameLabel`, `indexes.unique`, `indexes.columnsLabel`, `indexes.addColumn`, `indexes.chooseColumn`, `indexes.moveUp`, `indexes.moveDown`, `indexes.removeColumn`, `indexes.remove`, `indexes.lastColumn`.
...
- Test thêm khi làm (Vấn đề 64, 65): `table-panel.test.tsx` `does not dispatch an empty position`, `tags the name and comment fields with their issue paths`; `column-list.test.tsx` `tags the column fields with the issue paths they fix`, `opens the details of a column with an issue in them`, `opens the details when an issue appears in them`; `column-type-combobox.test.tsx` `searches enums by name and not by id`, `marks the current enum for screen readers`; `column-details.test.tsx` `keeps showing a stored default that the type no longer allows`, `ignores a length that is not a positive whole number`, `tags the type parameter fields with their issue paths`, `tags the checked default radio with the default value path`, `tags the literal value field with the default value path`; `index-list.test.tsx` `tags the index fields with their issue paths`.
...
- `relation-panel.tsx` (`"use client"`), props `{ readonly relationId: RelationId }`:
  ...
  - Nút "Xóa quan hệ" dispatch `removeRelation` rồi `setSelection(EMPTY_SELECTION)`. **Đã đổi ở Task 29 (Vấn đề 80):** props là `{ readonly relationId: RelationId; readonly onDelete: () => void }`; nút chỉ gọi `onDelete()`.
...
- **Đã đổi ở Task 29 (Vấn đề 80):** `removes the relation and clears the selection` thành `asks to delete the relation`; các test xóa của `multi-selection-panel.test.tsx` được thay bằng `asks to delete the whole selection` và chuyển sang `use-delete-selection.test.tsx`.
...
- Tạo trong `frontend/src/features/editor/components/dialogs/`: `create-relation-dialog.tsx`, `create-relation-dialog.test.tsx`, và các file tách ra khi làm (Vấn đề 71): `relation-draft-form.tsx`, `relation-draft-fields.tsx`, `relation-tables-field.tsx`, `foreign-key-fields.tsx`, `referenced-columns-field.tsx`, `column-pairs-field.tsx`, `junction-name-field.tsx`, `dialog-select-field.tsx`, `dialog-radio-field.tsx`, `field-error-message.tsx`, `use-relation-field-errors.ts`, `use-relation-field-errors.test.ts`, `relation-dialog-fields.ts`.
...
- `to-relation-draft.ts`:
  ...
  - Tài liệu không có đủ hai bảng, hoặc handle không parse được, thì trả `null`. **Đã đổi ở Task 29 (Vấn đề 82):** `createRelationDraftFromTable` không có bảng khác thì dùng chính bảng `from` làm `toTable` (quan hệ tự tham chiếu), và chỉ trả `null` với id bảng không có trong tài liệu.
- `build-relation-operation.ts`:

  ```ts
  export type RelationDraftError =
    | { readonly field: "referencedColumnIds"; readonly reason: "primary-key-missing" | "duplicate-column" }
  ...
  - `validateRelationDraft` chặn đúng ba trường hợp spec nêu: `referencedColumnIds` rỗng (1-n, 1-1) hoặc một trong hai bảng thiếu khóa chính (n-n); chế độ "Dùng cột có sẵn" còn cột được tham chiếu chưa ghép; một cột được chọn hai lần. Cột được tham chiếu lặp bị báo `referencedColumnIds` `duplicate-column` ở cả hai chế độ (Vấn đề 76). Khác kiểu và cột đích không unique **không** bị chặn.
  - `buildRelationOperation`:
    ...
    - `foreignKeyMode: "new-columns"` → `buildRelation(document, { fromTableId, toTableId, kind, onDelete, onUpdate, referencedColumnIds }, generateId)` của core, với `referencedColumnIds` của draft (Vấn đề 70); core lo đặt tên cột, `isUnique` cho 1-1 một cột và index unique cho 1-1 nhiều cột.
    - `foreignKeyMode: "existing-columns"` → một `addRelation` duy nhất với `columnPairs` của draft và id từ `createRelationId(generateId)`.
  - Comment: chế độ "Dùng cột có sẵn" không có cột nào cần thêm, nên dựng một `addRelation` từ các cặp đã chọn; chế độ "Tạo cột mới" giao các cột được tham chiếu cho `buildRelation`, hàm này thêm một cột khóa ngoại cho mỗi cột (Vấn đề 70).
- `create-relation-dialog.tsx` (`"use client"`), props `{ readonly draft: RelationDraft | null; readonly onClose: () => void }`:
  ...
  - Trường: bảng `from` (chỉ hiển thị, có nút "Đổi chiều" hoán đổi hai bảng và dựng lại giá trị mặc định), bảng `to` (`Select`, gồm cả bảng `from` để tạo được quan hệ tự tham chiếu; Vấn đề 76), loại (`RadioGroup` ba lựa chọn), cột được tham chiếu (`Select` nhiều dòng, chỉ hiện với 1-n và 1-1), chế độ khóa ngoại (`RadioGroup` "Tạo cột mới" hoặc "Dùng cột có sẵn"), danh sách cặp cột (chỉ hiện ở chế độ "Dùng cột có sẵn"), tên bảng trung gian (chỉ hiện với n-n).
  - Lỗi từ `validateRelationDraft` hiện ngay dưới trường tương ứng, nối bằng `aria-describedby`, được đọc qua một vùng `role="status"` `sr-only` luôn có mặt, và nút xác nhận `disabled` (Vấn đề 75).
  - Xác nhận: `buildRelationOperation` rồi `dispatch` **một lần**; lỗi `Result` thì hiện thông báo `errors:codes.<code>` với `role="alert"` và không đóng. Lỗi xếp theo `path`: `referencedColumnIds` hiện dưới "Cột được tham chiếu", còn lại dưới phần chọn bảng; lỗi cũ được xóa bằng `flushSync` trước khi xác nhận lại (Vấn đề 75). Thành công thì `onClose()`.
  - `onDelete`, `onUpdate` không có trên hộp thoại, luôn là `noAction`. `generateId` là `crypto.randomUUID()` trong component (Vấn đề 76).
  - Focus ban đầu: nút xác nhận khi đang bật; khi `disabled` thì control đầu tiên có `aria-invalid="true"`, không có thì nội dung hộp thoại (Vấn đề 74).
  - **Đã đổi khi làm (Vấn đề 72):** hộp thoại tự trả focus trong `onCloseAutoFocus`, không phải trong `onClose`, vì `FocusScope` của Radix hoàn tác focus đặt trong `onClose`. Mở bằng nút thì Radix trả focus về nút đó; mở bằng kéo thì focus `.react-flow__node` có `dataset.id` là bảng nguồn, node không còn thì `.react-flow` → `closest("[tabindex]")` (Task 29 dựng `<main tabIndex={-1}>`, Vấn đề 73). `onClose` của Task 29 chỉ xóa draft.
- Key `editor.relationDialog` cần có: `title`, `description`, `fromTable`, `toTable`, `swap`, `kindLabel`, `kind.oneToMany`, `kind.oneToOne`, `kind.manyToMany`, `referencedColumns`, `foreignKeyMode.label`, `foreignKeyMode.newColumns`, `foreignKeyMode.existingColumns`, `columnPairs.fromLabel`, `columnPairs.toLabel`, `junctionTableName`, `submit`, `errors.primaryKeyMissing`, `errors.unmatchedColumn`, `errors.duplicateColumn`; thêm khi làm (Vấn đề 71): `enterHint`, `referencedColumnLabel`, `columnPairs.title`, `columnPairs.placeholder`, `errors.primaryKeyMissingManyToMany` (`columnPairs.toLabel` là dòng gợi ý "references {{column}}").
...
- Test thêm khi làm (Vấn đề 70, 72, 74, 75, 76): `create-relation-dialog.test.tsx` `promises Enter in the description only when confirming is possible`, `blocks many-to-many when a table has no primary key`, `blocks a referenced column chosen twice`, `shows a rejected referenced column under the referenced columns`, `shows a repeated rejection as a new alert`, `shows a rejected many-to-many relation under the tables`, `creates new columns against the column the connection was dropped on`, `returns focus to the canvas region when the source node is gone`, `names the column pair and junction fields`, `reports an unpaired referenced column`, `reports no axe violations while blocked in the %s theme`, `reports no axe violations for many-to-many in the %s theme`; `build-relation-operation.test.ts` `reports a missing primary key of either table for many-to-many`, `reports a referenced column chosen twice with %s`, `references the dropped column when creating new columns`, `creates new columns against a target without a primary key`, `reports a referenced column of another table from core`, `reports a missing table`; `to-relation-draft.test.ts` `returns null for %s`, `returns null when the document has no other table` (Task 29 đổi thành `builds a self relation when the document has no other table` và thêm `returns null for a table the document does not hold`, Vấn đề 82), `swaps both tables and rebuilds the defaults`, `keeps the kind when the target table changes`, `moves the pair along when a referenced column changes`, `pairs a foreign key column with a referenced column`; `use-relation-field-errors.test.ts` `places an error on %s`.
...
- Sửa `frontend/src/features/editor/components/editor-workspace.tsx` và `frontend/src/features/editor/components/editor-workspace.test.tsx` (cả hai do Task 22 tạo; Vấn đề 63).
...
- Tạo `frontend/src/features/editor/hooks/use-delete-selection.ts`, `use-delete-selection.test.tsx`, `use-focus-request.ts`, `use-focus-request.test.tsx`, và file tách ra khi làm (Vấn đề 78): `use-workspace-keyboard.ts` (không có test riêng).
- Sửa `frontend/src/lib/i18n/locales/en/editor/editor-layout.ts`, `locales/vi/editor/editor-layout.ts`.
- Sửa khi làm, file của task trước: `components/panels/table-panel/table-panel.tsx`, `table-panel.test.tsx` (Task 26), `components/panels/relation-panel.tsx`, `relation-panel.test.tsx`, `multi-selection-panel.tsx`, `multi-selection-panel.test.tsx` (Task 27) cho đường xóa chung (Vấn đề 80); `lib/should-handle-shortcut.ts`, `should-handle-shortcut.test.ts` (Task 8, Vấn đề 81); `lib/to-relation-draft.ts`, `to-relation-draft.test.ts` (Task 28, Vấn đề 82). Mọi đường dẫn nằm dưới `frontend/src/features/editor/`.
...
- `use-delete-selection.ts`: `useDeleteSelection(): () => void`.
  ...
  - **Đã đổi khi làm (Vấn đề 79, 80):** chữ ký là `useDeleteSelection(focusCanvas: () => void): () => void`. Dispatch bị từ chối thì không xóa lựa chọn, không toast. Không lưu `previousHistoryLength`: ngay sau `dispatch`, hook giữ tham chiếu `history.past.at(-1)`, và "Hoàn tác" chỉ gọi `undo()` khi `history.past.at(-1)` vẫn là đúng mục đó. Đây là đường xóa duy nhất của editor: phím `Delete` và nút xóa của cả ba panel đều gọi hàm này.
- `use-workspace-keyboard.ts` (thêm khi làm, Vấn đề 78): `useWorkspaceKeyboard(isDialogOpen: boolean): { setCanvasRegion, deleteSelection }`, nối `useDeleteSelection`, `useEditorShortcuts` (`platformHint: navigator.platform`, an toàn vì `EditorScreen` nạp với `ssr: false`) và `useFocusRequest(focusFieldByPath)`.
- `use-focus-request.ts`: `useFocusRequest(onFocus: (path: DocumentPath) => void): void` — một effect theo dõi `focusRequest` của store; khác `null` thì gọi `onFocus(path)` rồi `requestFocus(null)`. Panel đã đặt `data-focus-path` (chuỗi `JSON.stringify(path)`, đúng đường dẫn issue của core) trên phần tử nhập liệu (Vấn đề 64), và hàm `onFocus` mặc định tìm phần tử theo thuộc tính đó rồi gọi `focus()`; không tìm thấy thì không làm gì (kể cả khi phần "Chi tiết" của cột đã bị người dùng đóng).
- `properties-panel.tsx` (`"use client"`): `<aside aria-label={t("propertiesLabel")}>` chọn nội dung theo lựa chọn, đúng bảng của spec mục 2:
  ...
  - **Đã đổi khi làm (Vấn đề 80, 83):** props `{ id, onCreateRelation, onDelete }`; `<aside id tabIndex={-1}>` là đích của skip link. Ba panel con nhận `onDelete` thay cho `onDeleted`: `<TablePanel tableId onCreateRelation onDelete />`, `<RelationPanel relationId onDelete />`, `<MultiSelectionPanel selection onDelete />`.
- `skip-to-panel-link.tsx`: link chỉ hiện khi focus (`sr-only focus:not-sr-only`), đưa focus tới panel thuộc tính, hoặc panel trái khi chưa chọn gì. Nội dung `t("skipToPanel")`. **Đã làm (Vấn đề 83):** props `{ propertiesPanelId, leftPanelId }`; khi chưa chọn gì, đích là `<div tabIndex={-1}>` bọc `<LeftPanel />`; link thêm `focus:px-3 focus:py-2` vì `focus:not-sr-only` đặt lại padding; `onClick` gọi `preventDefault` rồi `focus()` đích.
- `editor-workspace.tsx` (sửa từ Task 22) trở thành bố cục đầy đủ. Hiện workspace chưa có landmark nào (toolbar và canvas nằm trong một `<div>` xếp dọc); task này thêm `<header>`, hai `<aside>`, `<main>` và skip link (Vấn đề 63):
  - `<EditorStoreProvider>` → `<EditorFlowProvider>` (giữ như Task 22, Vấn đề 59) bọc **toàn bộ** bố cục, gồm cả panel trái (`useRevealTable` cần `useViewportControls`), panel phải và hộp thoại: `<header>` chứa `<EditorToolbar />`; một hàng gồm `<LeftPanel />` (`<aside>`), `<main>` chứa `<SkipToPanelLink />` và `<EditorCanvas />`, rồi `<PropertiesPanel />` (`<aside>`).
  - **Bắt buộc (Vấn đề 73):** vùng canvas là `<main tabIndex={-1}>` bọc `.react-flow`. Hộp thoại tạo quan hệ trả focus về phần tử `[tabindex]` gần nhất bao `.react-flow` khi node nguồn không còn; thiếu `tabIndex` thì focus rơi về `body`. Đây cũng là "vùng canvas" mà focus quay về sau khi xóa (spec mục 12).
  - Thứ tự tab đúng spec: toolbar → panel trái → canvas → panel phải, bằng chính thứ tự DOM; không dùng `tabIndex` dương.
  - **Đã làm:** DOM là `<header>` (toolbar); `<div tabIndex={-1}>` bọc `<LeftPanel />`; `<main tabIndex={-1} aria-label={t("layout.canvasLabel")}>` chứa skip link ở đầu và `<EditorCanvas />`; `<PropertiesPanel />` (`<aside>`); `<CreateRelationDialog />`. `<main>` và div bọc hiện vòng focus bằng lớp phủ `::after` (Vấn đề 83).
  - `useEditorShortcuts({ store, canvasElement, isDialogOpen, platformHint: navigator.platform, onDeleteSelection })` với `canvasElement` lấy từ ref của `<main>`, `isDialogOpen` là state của workspace (hộp thoại tạo quan hệ, đổi tên schema). **Đã đổi khi làm (Vấn đề 78, 81):** gọi qua `useWorkspaceKeyboard`; `isDialogOpen` chỉ phản ánh hộp thoại tạo quan hệ, còn hộp thoại đổi tên schema được chặn vì `shouldHandleShortcut` bỏ qua target nằm trong `[role="dialog"]`, `[role="alertdialog"]`.
  - `useFocusRequest(...)` và `<CreateRelationDialog draft onClose />`; `onConnect` của canvas (Task 22 để hàm rỗng) gọi `createRelationDraftFromConnection`, nút "Thêm quan hệ" của panel bảng gọi `createRelationDraftFromTable`. `onClose` chỉ đặt draft về `null`; hộp thoại tự trả focus (Vấn đề 72).
  - `onDeleted` của panel đưa focus về vùng canvas. **Đã đổi khi làm (Vấn đề 80):** workspace truyền `deleteSelection` làm `onDelete` của `PropertiesPanel`; hàm này tự focus `<main>`. Nút "Thêm quan hệ" mở được hộp thoại cả khi schema chỉ có một bảng (quan hệ tự tham chiếu, Vấn đề 82).
- Key `editor.layout` cần có: `skipToPanel`, `propertiesLabel`, `canvasLabel`, `deleted.one`, `deleted.many`. **Đã làm (Vấn đề 84):** `deleted.many` là cặp key số nhiều `many_one`, `many_other`; key `many_one` của `vi` chỉ để hai bộ key bằng nhau.
...
- `editor-workspace.test.tsx` (giữ các test Task 22 đã viết, Vấn đề 63; thêm):
  ...
  - `reports no axe violations in the %s theme` đã có từ Task 22; Task 29 giữ tên đó và cho nó chạy trên bố cục đầy đủ.
- Test thêm khi làm (Vấn đề 73, 79, 80, 81, 82, 83, 84, 85): `use-delete-selection.test.tsx` `keeps the selection when the deletion is rejected`, `counts the elements in the Vietnamese toast`, `leaves a later change alone when the toast action runs`; `properties-panel.test.tsx` `names the panel in %s`; `editor-workspace.test.tsx` `renders the canvas region as a focusable main around react flow`, `moves focus to the left panel through the skip link without a selection`, `does not undo while focus is on a button of the rename dialog`, `deletes the table from the table panel with an undo toast`; `should-handle-shortcut.test.ts` `ignores shortcuts from a button inside a %s element`; test đổi tên ở `table-panel.test.tsx`, `relation-panel.test.tsx`, `multi-selection-panel.test.tsx`, `to-relation-draft.test.ts` (Vấn đề 80, 82). Test của hook và workspace dùng `Toaster` thật và gọi `toast.dismiss()` trong `afterEach` (Vấn đề 85).
...
- `scripts/print-large-schema-snippet.ts`: in ra stdout một đoạn JavaScript tự chạy, dùng **API IndexedDB gốc** (không import Dexie), mở database `schemaforge`, ghi một bản ghi `schemas`, `documents` cho fixture chuẩn với `id` cố định, rồi in hướng dẫn tải lại trang. Script không ghi file, không gọi mạng. **Đã làm (Vấn đề 68):** fixture được nạp bằng `import type` cộng `import()` động một URL tính sẵn, kiểm bằng type guard; đoạn JavaScript dừng và in hướng dẫn khi database chưa tồn tại, nên phải mở màn hình danh sách schema một lần trước khi dán.
...
Mong đợi: script in ra đoạn JavaScript; `git diff --exit-code` trên lockfile thoát mã 0. Pipe qua `head -5` có thể in `EPIPE` vì `head` đóng pipe sớm; đó không phải lỗi của script (Vấn đề 68). Lệnh `node --experimental-strip-types` đã chạy được trên Node 24.
...
- **Gợi ý thêm sau đợt 13 (Vấn đề 85):** phủ đầu cuối nút xóa của panel quan hệ và panel nhiều lựa chọn (toast hiện, "Hoàn tác" khôi phục), vì Task 29 chỉ có test workspace cho nút xóa của panel bảng. Test render `Toaster` thì gọi `toast.dismiss()` trong `afterEach`.
...
2. **Checklist kiểm tra tay** trên bản production (`pnpm --filter @schemaforge/frontend exec next build && pnpm --filter @schemaforge/frontend start`), Chrome mới nhất, mở qua `http://localhost:3000`. Ghi kết quả từng mục vào báo cáo và vào PR:
   ...
   - Trên mọi route có import `@schemaforge/core` (`/`, `/schemas/<id>`), tải thẳng route đó (không đi qua điều hướng phía client) rồi xem Console: không có vi phạm CSP liên quan `unsafe-eval` (Vấn đề 43). Với `/schemas/<id>`, xem tới khi editor đã mở xong (chunk `editor-screen` nạp động sau `editor-screen-loader.tsx`), và làm cả với một schema không tồn tại.
   ...
   - Đổi tên schema bằng nút tên trên toolbar rồi đóng hộp thoại (xác nhận, Hủy, Escape): focus quay về đúng nút tên schema (Vấn đề 53).
   - Trên Safari với bộ gõ Telex và VNI: gõ tên bảng, tên cột có dấu, phím Enter kết thúc cụm gõ **không** commit ô nhập; Enter tiếp theo mới commit (Vấn đề 56).
   - Màn hình danh sách, chỉ dùng bàn phím trên Chrome: mở menu của dòng → chọn Đổi tên hoặc Xóa → hộp thoại → đóng: focus về đúng nút menu của dòng (hoặc `<h1>` khi dòng đã bị xóa, Vấn đề 49), và sau đó `<body>` không còn sót `pointer-events: none` (kiểm tra trong tab Elements).
   - Mở `/schemas/<id>` với VoiceOver: trạng thái "Đang mở schema…" được đọc **một** lần (không lặp lại ở mỗi bước lấy khóa, đọc tài liệu); ở màn hình trạng thái (không tìm thấy, đang mở ở tab khác, không đọc được, bộ nhớ không dùng được) và trang lỗi `error.tsx`, focus nằm trên `<h1>` và tiêu đề được đọc ngay (Vấn đề 60).
   - Ô chọn kiểu cột trong panel bảng: `CommandInput` tự dựng (Vấn đề 23) trông khớp các ô nhập khác ở cả hai theme, có vòng focus; mở popover thì focus nằm trong ô tìm kiếm, Escape trả focus về nút mở; mục "Kiểu tự đặt…" chuyển focus vào ô tên kiểu; VoiceOver đọc "(đang dùng)" ở mục đang chọn (Vấn đề 65). Nút icon lên, xuống, xóa trong panel bảng đo được 24×24 CSS px.
   - Phần "Chi tiết" của cột tự mở khi cột có issue về kiểu, giá trị mặc định hoặc comment, và VoiceOver đọc nhãn nút "Chi tiết (có vấn đề)" cùng thông báo lỗi của trường (Vấn đề 64).
   - Ở chế độ dev (`pnpm dev`, React StrictMode), mở một schema: màn hình "đang mở ở tab khác" có thể thoáng hiện rồi editor mở (Vấn đề 47). Ghi lại có thấy hay không; bản production (các mục trên) không được có hiện tượng này.
   - Độ tương phản bằng công cụ của DevTools trên cả hai theme: chữ thường ≥ 4.5:1; icon, viền, vòng focus ≥ 3:1. Đo đủ các cặp token chữ, nền của toolbar, panel, node, edge, minimap và toast, cùng màn hình trạng thái của editor và trang `error.tsx` (tiêu đề, mô tả `text-muted-foreground`, link `text-primary`, vòng focus của `<h1>` và link). Đo riêng chữ lỗi cỡ nhỏ dùng `text-destructive` (thông báo lỗi dưới ô nhập, trạng thái "Lưu thất bại") trên nền của nó ở cả hai theme: ≥ 4.5:1.
   ...
   - Hộp thoại "Tạo quan hệ" với VoiceOver (và NVDA nếu có): thông báo vì sao không xác nhận được khi mở hộp thoại đang bị chặn **không** bị đọc hai lần; mô tả lỗi "chọn một cột hai lần" không bị đọc hai lần; lỗi core khi xác nhận được đọc, và đọc lại khi xác nhận lần nữa (Vấn đề 75).
   - Kéo nối thật trên canvas để mở hộp thoại rồi đóng (xác nhận, Escape): focus về node nguồn. React Flow có thể `preventDefault` sự kiện `mousedown`, làm một ô trong panel vẫn giữ focus cũ và bị coi là phần tử đã mở hộp thoại; nếu vậy ghi lại focus rơi vào đâu (Vấn đề 72). Xóa node nguồn trong lúc hộp thoại mở thì focus về vùng canvas (Vấn đề 73).
   - Hộp thoại "Tạo quan hệ" ở cả hai theme: độ tương phản của chữ lỗi `text-destructive` và chữ gợi ý trên nền `bg-popover` ≥ 4.5:1, viền radio ≥ 3:1; vòng focus nhìn thấy rõ trên combobox đang `aria-invalid`; bấm chuột vào radio trúng đúng lựa chọn, vì vùng bấm mở rộng của các radio chồng lên nhau khoảng 8 px; nhãn `vi` dài không vỡ bố cục ở chiều rộng 320 px.
   - Vòng focus lớp phủ `::after` (Vấn đề 83) nhìn thấy trên `<main>` sau khi nhấn `Delete`, và trên đích panel trái sau khi dùng skip link lúc chưa chọn gì, ở cả hai theme, với độ tương phản ≥ 3:1 so với `--background`.
   - Skip link khi hiện có padding, cao ít nhất 24 px (Vấn đề 83).
   - Thứ tự Tab trên Chrome: toolbar → panel trái → skip link và canvas → panel thuộc tính.
   - Trình đọc màn hình đọc toast xóa cả khi xóa bằng phím `Delete` lẫn bằng nút trong panel, và Alt+T tới được nút "Hoàn tác" (Vấn đề 80).
   - Focus trên nút Hủy của hộp thoại đổi tên schema: `Ctrl/Cmd+Z` không làm gì (Vấn đề 81).
   - Toast không bao giờ che hoàn toàn một node đang focus.
   - Yêu cầu focus từ tab "Vấn đề" cuộn panel thuộc tính để trường được thấy trọn.
   - Chiều rộng panel thuộc tính với nội dung tiếng Việt dài (`ScrollArea` của Radix dùng `display: table`).
   - Số đo hiệu năng trên fixture chuẩn (nạp bằng `pnpm --filter @schemaforge/frontend perf:snippet`; mở màn hình danh sách schema một lần trước khi dán đoạn JavaScript vào Console của Chrome, Vấn đề 68), đo theo spec mục 13 "Cách đo tay", CPU chậm 4×: INP của commit tên cột, undo, redo ≤ 200 ms; kéo một bảng trung bình ≥ 30 fps; mở schema tới lúc tương tác được ≤ 2 s. Không đạt thì bật `onlyRenderVisibleElements` rồi đo lại, và báo cáo cả hai lần đo.
